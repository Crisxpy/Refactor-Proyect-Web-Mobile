...
const obj2 = {};
const obj3 = {};
let a, b, cc, dd, ee, ff, gg, hh, ii2, jj2;
let benja;
// =====================================
...
function doEverything(u, p2, action, dat, extraDat, moreData, flag99, cb) {
  ...
  if (action == "buscarProductos") {
    const query = dat;
    const cat = extraDat;
    const minP = moreData ? moreData.min : 0;
    const maxP = moreData ? moreData.max : 999999999;
    const res = [];
  ...
  if (action == "checkout") {
    const userId3 = dat;
    const metodoPago = extraDat;
    const direccion = moreData;
    let foundUser2 = null;
    for (var i = 0; i < dbUsers.length; i++) {
    ...
    for (var i = 0; i < foundUser2.carrito.length; i++) {
      for (var j = 0; j < dbProducts.length; j++) {
        if (dbProducts[j].id == foundUser2.carrito[i].prodId) {
          const itemTotal = dbProducts[j].prec * foundUser2.carrito[i].qty;
...
function search(q, filters) {
  ...
  const results = [];
  for (let ii = 0; ii < prods.length; ii++) {
...
function processRegistrationFormAndValidateAndSaveAndSendEmailAndLoginAndRedirect(
  ...
  let exists = false;
  const usersDB = [{ email: "juan@mail.com" }, { email: "maria@mail.com" }]; // hardcoded again
...
function updateUserProfile(
  ...
  let user4 = null;
  for (let i = 0; i < dbUsers3.length; i++) {
...
function reviews(action3, prodId3, userId5, rating2, comment, data4) {
  ...
  if (action3 == "add") {
    ...
    const compro = false; // siempre false en este ejemplo - logica incompleta
...
function formatDate(d4) {
  let day = d4.getDate();
  let month = d4.getMonth() + 1;
  const year = d4.getFullYear();
  let hours = d4.getHours();
  let mins = d4.getMinutes();
  let secs = d4.getSeconds();
  if (day < 10) day = "0" + day;
...
function formatDate2(d5) {
  ...
  let day2 = d5.getDate();
  let month2 = d5.getMonth() + 1;
  const year2 = d5.getFullYear();
  ...
  if (op == "flatArray") {
    ...
    for (var i = 0; i < val.length; i++) {
      if (Array.isArray(val[i])) {
        for (let j = 0; j < val[i].length; j++) f.push(val[i][j]);
      } else f.push(val[i]);
    }
    return f;
  }
